...
final class AdapterImpl implements View.OnClickListener, View.OnLongClickListener {
  ...
  private final Map<Integer, ObjectPool<PhotoGridViewHolder<?>>> viewHoldersMap = new ArrayMap<>();
  ...
  void onBindViewHolder(ViewHolder holder, int position, ViewGroup parent) {
    ...
    while (!rowItems.isEmpty() && columnIndex < listView.getNumColumns()) {
      ...
      if (spaceLeftInColumn >= currentItem.getItem().getRowSpan()) {
        ...
        ObjectPool<PhotoGridViewHolder<?>> pool = viewHoldersMap.get(viewType);
        if (pool == null) {
          pool = new ObjectPool<>();
          viewHoldersMap.put(viewType, pool);
        }
        PhotoGridViewHolder viewHolder = pool.get();
        if (viewHolder == null) {
    ...
    if (debugEnabled && position % 20 == 0) {
      ...
      for (Map.Entry<Integer, ObjectPool<PhotoGridViewHolder<?>>> e : viewHoldersMap.entrySet()) {
  ...
  int getRowHeight(PhotoGridItem item) {
  ...
  int getRowWidth(PhotoGridItem item) {
  ...
  private LinearLayout initializeLayout(LinearLayout layout) {
    ...
    for (int j = 0; j < childCount; j++) {
      ...
      for (int k = 0; k < innerChildCount; k++) {
        ...
        ObjectPool<PhotoGridViewHolder<?>> pool = viewHoldersMap.get(viewState.viewType);
  ...
  private static class ViewState {
    ...
    private final PhotoGridViewHolder<?> viewHolder;

    private ViewState(int viewType, RowItem rowItem, PhotoGridViewHolder<?> viewHolder) {
