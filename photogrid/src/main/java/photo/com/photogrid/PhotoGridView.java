...
public class PhotoGridView extends ListView implements AsymmetricView {
  protected OnItemClickListener onItemClickListener;
  protected OnItemLongClickListener onItemLongClickListener;
  protected PhotoGridViewAdapter gridAdapter;
  private final PhotoGridViewImpl viewImpl;

  public PhotoGridView(Context context, AttributeSet attrs) {
    super(context, attrs);

    viewImpl = new PhotoGridViewImpl(context);
  ...
  @Override public void setAdapter(@NonNull ListAdapter adapter) {
    if (!(adapter instanceof PhotoGridViewAdapter)) {
      throw new UnsupportedOperationException(
          "Adapter must be an instance of AsymmetricGridViewAdapter");
    }

    gridAdapter = (PhotoGridViewAdapter) adapter;
  ...
  @Override public void onRestoreInstanceState(Parcelable state) {
    if (!(state instanceof PhotoGridViewImpl.SavedState)) {
      super.onRestoreInstanceState(state);
      return;
    }

    PhotoGridViewImpl.SavedState ss = (PhotoGridViewImpl.SavedState) state;
